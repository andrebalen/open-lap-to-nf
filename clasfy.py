...
def thresholding__transform(im, thresholdMode, threshold, maxvalue, combinedWithOTSU):
    imgray = im.copy()
    if im.ndim > 2:
...
def mser__transform(im, minArea, maxArea, max_variation, min_diversity,  max_evolution, area_threshold, min_margin, blur_size, source):
    copiedim = im.copy()
    img2     = im.copy()
